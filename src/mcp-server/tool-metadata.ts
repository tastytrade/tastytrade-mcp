...
export const TOOL_METADATA: Record<string, ToolMeta> = {
  ...
  tastytrade_dry_run_order: {
    ...
    description:
      "Read-only pre-flight. Validates a prospective single- or multi-leg order via POST /accounts/{account_number}/orders/dry-run WITHOUT routing it; moves no money. Use it before tastytrade_place_order to preview buying-power-effect, estimated fees, and any blocking errors / non-blocking warnings, and to mint the `confirmation_token` required to actually submit. Returns the full unwrapped PlacedOrderResponse (order, buying-power-effect, fee-calculation, closing-fee-calculation, warnings[], errors[], notes[]) PLUS a `confirmation_token` field: a non-null single-use ~60s token is issued ONLY when the dry-run reports no errors AND describes an order (it carries an `order`, a `complex-order`, or a `buying-power-effect`), and it is bound to the exact {account_number, order body}. A null token has a SECOND cause with a different fix: a dry-run that reported no errors but described no order — no `order`, no `complex-order`, no `buying-power-effect` — also mints nothing, and there is no error text to act on. Retrying the identical dry-run will not help; check that TASTYTRADE_API_URL names the real API and that nothing between this server and the broker is rewriting the response body. If errors[] is non-empty the token is null and you must fix the order and re-run; treat any non-empty errors[] as 'do not proceed', and treat warnings[] as informational signals that the live order could still be rejected. Re-run this whenever any arg changes before submitting. Common isError (structured ToolError): invalid leg action for instrument type (code 'validation'), market-data/validation failures, 'rate_limit_exceeded' (the 50/sec global rate limit).",
    paramDescriptions: {
      account_number:
    ...
    outputSchema: {
      ...
      properties: {
        ...
        confirmation_token: {
          type: ["string", "null"],
          description:
            "Single-use ~60s token for tastytrade_place_order, bound to {account_number, order body}. Non-null ONLY when the dry-run reports no errors AND describes an order (an `order`, a `complex-order`, or a `buying-power-effect`); null otherwise — a non-empty errors[] tells you what to fix; a dry-run that described no order gives you nothing to fix and is a configuration or proxy problem, not an order problem.",
        },
        sanity_warnings: {
          type: "array",
          description:
      ...
      // that needs saying. A dry-run's 200 can carry a populated `errors[]` and
  ...
  tastytrade_dry_run_replace_order: {
    ...
    description:
      "Read-only pre-flight for a FULL order replacement via POST /accounts/{account_number}/orders/{id}/dry-run; routes nothing and moves no money. The body matches tastytrade_replace_order (order-level fields only — legs are NOT sent and are retained from the original order). Use before tastytrade_replace_order to preview buying-power impact, estimated fees, and blocking errors / non-blocking warnings, and to mint the `confirmation_token` (bound to action 'replace_order', single-use, ~60s TTL, bound to {account_number, order_id, body}). Returns the full unwrapped PlacedOrderResponse (order, buying-power-effect, fee-calculation, closing-fee-calculation, warnings[], errors[], notes[]) PLUS a `confirmation_token` field that is non-null ONLY when the dry-run reports no errors AND describes an order (otherwise null — fix the body and re-run; non-empty errors[] means do not proceed). A null token has a SECOND cause with a different fix: a dry-run that reported no errors but described no order — no `order`, no `complex-order`, no `buying-power-effect` — also mints nothing, and there is no error text to act on. Retrying the identical dry-run will not help; check that TASTYTRADE_API_URL names the real API and that nothing between this server and the broker is rewriting the response body. Common isError (structured ToolError): order not found ('not_found', 404), validation failures, 'rate_limit_exceeded' (the 50/sec global rate limit).",
    paramDescriptions: {
      account_number:
        "The tastytrade account number that owns the order. Required.",
      order_id:
    ...
    outputSchema: {
      ...
      // dry-run that reports only `errors[]` is the payload the agent is told
  ...
  tastytrade_dry_run_edit_order: {
    ...
    description:
      "Read-only pre-flight for a PARTIAL order edit via POST /accounts/{account_number}/orders/{id}/dry-run; routes nothing and moves no money. REQUIRED: order_type and time_in_force — the dry-run endpoint re-validates the FULL order shape, so a price-only body is rejected (400 validation_error: order-type/time-in-force missing). Also set the fields you intend to change (price, price_effect, stop_trigger, gtc_date). Note the live tastytrade_edit_order it precedes is implemented as cancel-replace, so applying the edit creates a NEW order id. Use this before tastytrade_edit_order to preview buying-power impact, estimated fees, and blocking errors / non-blocking warnings, and to mint the `confirmation_token` (bound to action 'edit_order', single-use, ~60s TTL, bound to {account_number, order_id, partial body}). Returns the full unwrapped PlacedOrderResponse (order, buying-power-effect, fee-calculation, closing-fee-calculation, warnings[], errors[], notes[]) PLUS a `confirmation_token` field that is non-null ONLY when the dry-run reports no errors AND describes an order (otherwise null — fix the fields and re-run; non-empty errors[] means do not proceed). A null token has a SECOND cause with a different fix: a dry-run that reported no errors but described no order — no `order`, no `complex-order`, no `buying-power-effect` — also mints nothing, and there is no error text to act on. Retrying the identical dry-run will not help; check that TASTYTRADE_API_URL names the real API and that nothing between this server and the broker is rewriting the response body. Common isError (structured ToolError): order not found / uneditable ('not_found' 404 / 'validation' 422), 'rate_limit_exceeded' (the 50/sec global rate limit).",
    paramDescriptions: {
      account_number:
        "The tastytrade account number that owns the order. Required.",
      order_id:
